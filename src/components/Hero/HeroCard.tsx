...
export function HeroCard() {
  ...
  useEffect(() => {
    // Start animation after a delay to match the staggered entrance
    const timer = setTimeout(() => {
      setIsVisible(true);
    }, 1700); // This matches the card timing from Hero.tsx
  ...
  return (
    <div className="relative sm:absolute right-0 sm:right-4 md:right-6 lg:right-8 bottom-0 sm:bottom-8 md:bottom-11 lg:bottom-13 flex flex-col items-end gap-2 sm:gap-3 md:gap-4 self-end">
      {/* Happy clients badge */}
      <div className={`flex items-center gap-1.5 sm:gap-2 transition-all duration-500 ease-out ${
      ...
      {/* Professional card */}
      <div 
        className={`relative bg-white/10 backdrop-blur-sm rounded-xl sm:rounded-2xl w-52 sm:w-60 md:w-64 lg:w-72 border border-white/20 bg-cover bg-center overflow-hidden transition-all duration-700 ease-out transform-gpu ${
          isVisible 
      ...
      >
        ...
        <div className={`relative h-full p-4 sm:p-5 md:p-6 flex flex-col justify-between transition-all duration-500 ease-out delay-300 ${
