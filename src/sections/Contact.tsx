...
export function Contact() {
    ...
    return (
        <section
            id="contact"
            ref={containerRef}
            className={`relative min-h-screen py-16 sm:py-24 px-4 sm:px-8 lg:px-8 flex flex-col justify-between ${isArtMode ? 'bg-black text-white' : 'bg-white text-black'} transition-colors duration-500 overflow-hidden`}
        >
            {/* Animated Background */}
            <motion.div
                style={{ y: backgroundY }}
            ...
            >
                ...
                <div className="relative pt-8 pb-0">
                    ...
                    <motion.div
                        className={`mt-4 w-full border-t border-b overflow-hidden flex flex-col md:flex-row ${isArtMode ? 'border-white/30' : 'border-black/30'}`}
                        variants={itemVariants}
                    >
                        {/* Description / Context */}
                        <div className={`flex-1 py-4 sm:py-6 md:py-8 md:pr-12 flex items-center ${isArtMode ? 'border-white/30' : 'border-black/30'} border-b md:border-b-0 md:border-r`}>
                ...
                >
                    <div className="w-full">
                        ...
                        <div className={`mt-6 sm:mt-8 flex flex-col md:flex-row justify-between items-center text-[8px] sm:text-[10px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-center md:text-left gap-3 sm:gap-4 md:gap-0 ${isArtMode ? 'text-white/30' : 'text-black/30'}`}>
